...
long long fibonacci(long long input, int level) {
    if (input < 2) return input;

    if (level < cutoff) {
        hpx::future<long long> n1 = hpx::async(fibonacci, input - 1, level + 1);
        long long n2 = fibonacci(input - 2, level + 1);
        return n1.get() + n2;
    } else {
        long long n1 = fibonacci(input - 1, level + 1);
        long long n2 = fibonacci(input - 2, level + 1);
        return n1 + n2;
    }
}
...
int hpx_main(hpx::program_options::variables_map& vm) {
    ...
    long long result = fibonacci((long long) fibNumber, 0);
